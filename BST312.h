...
{
    if(newItem > t->data){ //if more than, go right
        if((t->right) == NULL){
            //cout << "inserting on right" << endl;
            TreeNode *temp = new TreeNode();
            temp->left = NULL;
            temp->right = NULL;            
            temp->data = newItem;
    ...
    if(newItem < t->data){ //if less than go left
        if((t->left) == NULL){
...
{
    vector<ItemType> preOrder = vector<ItemType>();
    if(root != NULL){
...
{
    vector<ItemType> inOrder = vector<ItemType>();
    if(root != NULL){
...
{
    vector<ItemType> postOrder = vector<ItemType>();
    if(root != NULL){
...
{
    vector<ItemType> items = inOrderTraversal();
